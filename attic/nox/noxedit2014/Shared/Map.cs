...
namespace NoxShared
{
	public class Map
	{
        ...
        public class Waypoint
        {
            ...
            public string ShortName
            {
            	get
            	{
                    ...
                    // убираем название карты и двоеточие
		...
		{
			public short EntryVer = 2; // если 3 - игнорируем лимит игроков
		...
		public class Wall : IComparable
		{
			...
			/*** ДЛЯ СЕКРЕТНЫХ СТЕН ***/
			public int Secret_OpenWaitSeconds = 3; // Сколько _секунд_ стена ждет прежде чем снова подняться/опуститься
			public byte Secret_ScanFlags = 0; // 4 - стена сама опускается, 8 - стена сама поднимается
			public byte Secret_WallState = 0; // 4 - стена опускается, 3 - стена готовиться опуститься, 2 - стена поднимается, 1 - стена поднята
			public byte Secret_OpenDelayFrames = 0; // Сколько времени (фреймов) стена открывается/закрывается
			...
			public enum SecretScanFlags : byte
			{
				...
				AutoOpen = 2,
				AutoClose = 4,
				Unknown8 = 8 // помоему ставится игрой когда игрок рядом
			}
		...
		public class Object : IComparable, ICloneable
		{
			...
			/// <summary>
			/// Возвращает true если обьект можно поворачивать (используется в редакторе)
			/// </summary>
			...
			internal void Write(Stream stream, IDictionary toc)
			{
				...
				if (Terminator != 0)
				{
					...
					foreach (UInt32 u in SlaveGIDList)
						wtr.Write(u);
					// AngryKirC: используется только в сохранениях одиночной игры
					wtr.Write(AnimFlags);
					if (ReadRule2 >= 0x3F)
					{
						wtr.Write((short) 1);
		...
		{
			int length = (int) rdr.ReadInt64();
			SectHeader hed = new SectHeader("ScriptObject", rdr.ReadInt16());
			Headers.Add(hed.Name, hed);
			
			Scripts.NCObj = rdr.ReadBytes(rdr.ReadInt32());
		}
		#endregion
		...
		private void WriteScriptObject(NoxBinaryWriter wtr)
		{
			...
			wtr.SkipToNextBoundary();
			int pos = (int) wtr.BaseStream.Position;
			wtr.Write((long) 0); // Length of entire section
			wtr.Write(hed.Version);
			
			// Write nc.obj (compiled script stuff)
			wtr.Write(Scripts.NCObj.Length);
			wtr.Write(Scripts.NCObj);
			
			// Rewrite section length
			long length = wtr.BaseStream.Position - (pos + 8);
			wtr.Seek(pos, SeekOrigin.Begin);
			wtr.Write(length);
			// Back to end of stream
			wtr.Seek(0, SeekOrigin.End);
		}
		#endregion
	}
