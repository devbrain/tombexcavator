...
namespace archive::explode
{
	...
	{
	public:
		byte_reader(archive::input& input, uint32_t header_length)
			: m_input(input),
			m_header_length(header_length)
		{
		}

		explicit byte_reader(archive::input& input)
			: m_input(input),
			m_header_length(0)
		{
		}
	...
	{
	public:
		bit_reader(archive::input& input, uint32_t header_length)
		...
		explicit bit_reader(archive::input& input)
			: byte_reader(input),
			m_word(0),
			m_count(0)
		{
	...
	{
	public:
		struct_reader(archive::input& input, uint32_t header_length)
			: byte_reader(input, header_length)
		{
		}

		explicit struct_reader(archive::input& input)
			: byte_reader(input)
		{
		}
