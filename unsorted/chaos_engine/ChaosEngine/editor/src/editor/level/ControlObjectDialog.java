...
import base.layer.ControlObjectLayer;
...
public class ControlObjectDialog extends JDialog implements DataModifier<ControlObject> {
	...
	public void gui_to_data() throws Exception
	{
		...
		{
			m_control_object.set_name(getJNameTextField().getText());
		}
		
		m_control_object.set_type(getJTypeComboBox().getSelectedIndex());

		m_control_object.set_width(Integer.parseInt(getJWidthTextField().getText()));
		m_control_object.set_height(Integer.parseInt(getJHeightTextField().getText()));
		m_control_object.set_x(Integer.parseInt(getJXTextField().getText()));
		m_control_object.set_y(Integer.parseInt(getJYTextField().getText()));
	...
	public void on_close() 
	{
		try
		{
			...
			if ((!m_control_object.get_name().equals("")) ||
					(m_control_object.get_type() == ControlObject.Type.Foreground_Block)
					|| (m_control_object.get_type() == ControlObject.Type.Background_Block))
			{				
				m_observable.signal();
				dispose();
			}
