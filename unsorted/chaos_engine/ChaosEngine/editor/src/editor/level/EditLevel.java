...
import base.*;
import base.associations.*;
import editor.*;
import sys.ImageLoadSave;
...
public class EditLevel extends JPanel  implements MouseListener, KeyListener, MouseMotionListener, Observer
{
	...
	{
		return create_add_item_sub_menu(popup, "Create Token", x, y, GfxFrameSet.BONUSES, 
	...
	{
	JOptionPane.showMessageDialog(m_parent, message, title,
			JOptionPane.ERROR_MESSAGE);
	}

	
	public enum PopupMenuEditActionType { New_Tile, New_Tile_Set, Delete_Tile, Copy_Tile, 
		New_Object,Delete_Object,Edit_Object_Name, Edit_Object_Properties, 
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			case Derive_Associated_Trigger:
				...
				if (co.get_type() == ControlObject.Type.Enemy)
				{
					...
					ControlObject trigger = m_data.set_control_object(m_x + m_data.get_grid().get_tile_width(),
			...
			case Edit_Object_Properties:
				...
				if (go.get_source_set().get_type() != GfxFrameSet.Type.background_item)
				{
					choice = JOptionPane.showConfirmDialog(m_parent, go.is_visible() ? 
							"Should the object still be shown ?" : "Should the object now be shown", 
							"Properties for "+go.get_name(),JOptionPane.YES_NO_OPTION);
			...
			case Paste_Object:
				...
				{		
					...
					if (!check_duplicate(go,name))
					{
						...
						{
							if (!name.equals("") || 
									((ControlObject) go).get_type() == ControlObject.Type.Foreground_Block || 
									((ControlObject) go).get_type() == ControlObject.Type.Background_Block)
							{
								co = (ControlObject) go;
			...
			case New_Control_Object:
				...
				if (name != null)
				{	
					if (name.equals("") && (tt != ControlObject.Type.Foreground_Block))
					{
						error_message("Instance naming error","Control object should have a name");											
					}
					else
		...
		if (m_image != null)
		{
			...
			{		
				...
				else if (gfo != null)
				{
					...
					popup.add(new JSeparator(JSeparator.HORIZONTAL));
					create_popup_menu_item(popup,x,y,PopupMenuEditActionType.Edit_Object_Name);
	...
	{
		if ((arg1 instanceof ControlObjectDialog) || (arg1 instanceof HostileWaveDialog))
		{
			repaint();
			m_data.set_modified(true);
		}
		...
		else if (arg1 instanceof LevelImportDialog)
		{
			...
			BufferedImage leftovers = m_data.import_from_image(image_file,lid.get_tolerance());
			...
			if (leftovers != null)
			{
				...
				{
					MiscUtils.show_error_dialog(this, "I/O error", "Failed to write "+leftover_file);
				}
			}
	...
	public EditLevel(ChaosEngineEditor parent,LevelData levelData)
	{
