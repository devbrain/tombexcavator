...

{
	...
	public void editor_render(Graphics g) 
	{
		...
		{
			m_control_layer.set_hostile_parameters(get_hostile_params(),get_level_palette());
	...
	public void link_objects_around(GfxObject centre, int distance, String prefix)
	{
		...
		for (GfxObject go : m_object_layer.get_items())
		{
			...
			{
				if ( (prefix == null) && (go.get_name().startsWith(GfxObject.UNNAMED_PREFIX)))
				{
					centre.link_object(go);
				}
				else if ((prefix != null) && (go.get_name().startsWith(prefix)))
				{
					centre.link_object(go);
				}
			}
	...
	private boolean line_test(int x, int y, int len, boolean vertical, Collection<? extends Locatable> c)
	{
		boolean rval = false;
	...
	{		
		...
		boolean game_scale = !gfx_mode.equals(EditableData.GfxMode.EDITOR);
		
		m_object_layer = new GfxObjectLayer(fr,m_level,m_tile_grid.get_tile_height()/2,this,game_scale);
		
		debug("loading control objects");
		
		m_control_layer = new ControlObjectLayer(fr,m_tile_grid.get_tile_width(),m_tile_grid.get_tile_height()/2,game_scale);
	...
	public BufferedImage import_from_image(String image_file, int rgb_tolerance)
	{
		...
		{			
			...
			rval = m_tile_grid.import_from_image(bi,m_level,rgb_tolerance);			
		}
		return rval;
	}
