...
public abstract class GameState
{
	...
	protected abstract void p_render(Graphics2D g);
	 ...
	 public class OptionControls
	 {
		 ...
		 public boolean is_jump_pressed()
		 {
			 boolean rval = m_game.is_key_pressed(KeyEvent.VK_SHIFT);
			 if (!rval && m_joystick != null)
			 {
				 int button_state = m_joystick.getButtons();
				 ...
				 if (button_state != last_button_state)
				 {
					 rval = (button_state & Joystick.BUTTON2) != 0;
	...
	protected boolean is_fire_pressed()
	{
		return m_controls.is_fire_pressed();
	}
  ...
  protected GameState() 
  {
	  if (WHITE_BIG_FONT == null)
	  {
		  try
		  {    	   
			  WHITE_BIG_FONT = new GameFont(DirectoryBase.get_font_path()+"big_letters",Color.WHITE,0xFF393925);
			  GOLDEN_BIG_FONT = new GameFont(DirectoryBase.get_font_path()+"big_letters");
			  GOLDEN_SMALL_FONT = new GameFont(DirectoryBase.get_font_path()+"small_letters");
			  GODS_GOLDEN_FONT = new GameFont(DirectoryBase.get_font_path()+"gods_letters");
			  GODS_WHITE_FONT = new GameFont(DirectoryBase.get_font_path()+"gods_letters",Color.WHITE,0);
		  }
		  catch (IOException e)
		  {
		  }
    ...
    if (m_error_message == null) 
    {
    	...
    	if (m_fadeout) 
    	{
    		set_color_ratio(1 - ( (m_state_elapsed - m_fadeout_start_time) /
    				(float) (m_fadeout_time - m_faded_out_delay)));
