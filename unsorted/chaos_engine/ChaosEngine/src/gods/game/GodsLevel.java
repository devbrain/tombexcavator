...
{	
	...
	{
		Locatable top_left_corner = m_level_data.get_control_object(corner_name);
	...
	protected void summon_shopkeeper(String [] items)
	{
	...
	protected boolean display_health_and_lives_bonus(String bonus_name,int min_nb_lives)
	{
		boolean rval = ((m_hero.get_nb_lives() >= min_nb_lives) && 
				(m_hero.get_health(false) >= (3*m_hero.get_max_health())/4));
		if (rval)
		{
			rval = display(bonus_name);
	...
	protected void set_hero_position(Locatable location, boolean see_above,
	...
	protected boolean zone_contains(String zone_name, String object_name)
	{
		...
		if (co != null)
		{
			co.get_bounds(m_work_rectangle);
			GfxObject go = m_level_data.get_bonus(object_name);
			if (go != null)
			{
				rval = (m_work_rectangle.contains(go.get_x_center(),go.get_y_center()));
	...
	protected void link_objects_around(String location, int distance, String prefix)
	{
		ControlObject co = m_level_data.get_control_object(location);
		if ( co != null)
		{
			m_level_data.link_objects_around(co,distance,prefix);
		}
	}
	...
	protected boolean display_help_health_bonus(String bonus_name)
	{
		...
		if (m_hero.get_health(false) < (m_hero.get_max_health() / 3))
		{
			display(bonus_name);
			print("help bonus");
			rval = true;
	...
	protected void show_display_animation(Locatable l)
	{
	...
	{
		return create_bonus(x,y,class_name,null);
	}

	protected GfxObject create_bonus(int x, int y, String class_name, String instance_name)
	{
		String name = instance_name;

		if (name == null)
		{
			name = "_bonus_"+m_bonus_index;
			m_bonus_index++;
		}
	...
	private Locatable m_scroll_lock_top_corner = null;
	...
	private void update_hero()
	{
		...
		case ALIVE:
			...
			Sector<GfxObject>.Distance closest_object = null;
			GfxObject go = null;
			...
			// check if bonus close to hero (auto-picked category)
				...
				closest_object = m_level_data.bonus_sector_set.closest(m_hero);
				...
				if (go != null)
				{
					...
					if (go.may_be_taken())
					{
						...
						{								
							...
							// bug: removed object boss_w3_world_key
			...
			closest_object = m_level_data.items_sector_set.closest(m_hero);
			...
			if (go != null)
			{
				...
				if (m_work_rectangle.intersects(m_work_rectangle_2))
				{
					...
					handle_chest_key_association(go);
				}
				else
				{
			...
			if (m_work_rectangle.intersects(m_work_rectangle_2))
			{
					...
					if (activated)
					{
						...
						if (oa != null)
						{
							if (oa instanceof LeverDoorKeyAssociation)
							{
								...
								if ((ldka.get_key() != null) && !m_hero.owns(key_name))
								{
							...
							else if (oa instanceof LeverPlatformKeyAssociation)
							{
								...
								if ((ldka.get_key() == null) || m_hero.steal(key_name))
								{
									// a key is required and the hero owns it
				...
				else
				{
					...
					if (just_activated)
					{
						...
						if (oa != null)
						{
							if ((!state.activated) && (oa instanceof LeverDoorAssociation))
							{
								// open when button is triggered
								LeverDoorAssociation lda = (LeverDoorAssociation)oa;
	...
	private void handle_chest_key_association(GfxObject go)
	{
		ObjectAssociation oa = m_level_data.get_association_set().get(go.get_name());
		if (oa instanceof ChestKeyAssociation) 
		{
			ChestKeyAssociation cka = (ChestKeyAssociation) oa;
		
			for (int i = 0; i < cka.get_nb_keys(); i++)
			{
				String key_name = cka.get_key(i).get_name();

				// check if not already open
				
				if ((m_hero.owns(key_name)) && (go.get_current_frame() == 1))
				{
					...
					// take key from hero now that chest is open

					m_hero.steal(key_name);

					// add points due to chest opening

					m_hero.add_score(go.get_properties().points,true);
	...
	{		
		...
		case LOADED:
			...
			{
				String level_name = m_level_set.get_name().toUpperCase().replace('_',' ') +
				" " + Localizer.value("LEVEL")+" "+m_level_set.get_level_index();
	...
	private void level_end()
	{
		...
		m_timer_events.add(new LevelEndEvent());
	...
	protected void game_end(boolean first_pass)
	{
		if (first_pass)
		{
			m_timer_events.add(new GameEndEvent());
		}
		else
		{
	...
	protected void p_update() 
	{
		...
		case RUNNING:
			...
			{
				if (m_hero.is_standing_on_ground())
				{
					...
					(m_boss.get_life_state() == LifeState.EXPLODING))
			{
				...
				boss_co.spawn_linked_objects(m_level_data);
		...
		case GAME_END:
			if (is_fadeout_done())
			{
				...
				if (completed)
				{
					...
					set_next(new GodsEndScreen(m_level_data.get_common_palette()));
				}
				else
				{
					try 
	...
	private void handle_restarts()
	{
		...
		if (closest != null)
		{
			...
			if (m_work_rectangle.intersects(m_work_rectangle_2))
			{
				...
				if ((!m_player_start_entered) && (closest.get_name().equals("player_start")))
				{
			...
			{	
				...
				if (oa != null)
				{
					...
					case Trigger_Close_Door:
					{
						...
						close_door(tid.get_door_to_close().get_name());
						closest.set_visible(false);
					}
					break;
					case Trigger_Item_Display:
						...
						if (m_hero.steal(tid.get_must_own().get_name()))
						{
							display(tid.get_item_to_display().get_name());
							closest.set_visible(false);
						}
	...
	private void load_level() throws IOException
	{
		...
		if (is_first_level)
		{
			m_hero = new Hero(m_game,get_joystick());
		}
		...
		m_hero.level_init(m_level_data,m_monster_layer,m_status_bar,m_sfx_set,m_view_bounds);
		...
		for (Hostile h : items)
		{
			if (h.get_name().equals("boss"))
			{
				...
				m_level_data.link_objects_around(co, co.get_height(), "boss_");
				break;
			}
		}
		...
		if (!temp_start)
		{
			if (is_first_level)
			{
				if (m_sold_items != null)
				{
					...
					m_hero.set_shield_level(m_level_data.get_shield_level());
					show_shop();
					m_sold_items = null;
				}
			}
		...
		try {
			...
			create_copper_effect(cb.create());
