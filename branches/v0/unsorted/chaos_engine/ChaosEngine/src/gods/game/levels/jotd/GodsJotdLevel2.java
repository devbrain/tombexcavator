...
public class GodsJotdLevel2 extends GodsLevel 
{
	...
	protected void on_lever_activated(GfxObject lever,
		...
		{
			if (m_hero.owns("w1_water_gem") || m_hero.owns("w1_fire_gem"))
			{
		...
		{
			if ((!m_w3_shortcut_moved) && (m_hero.owns("w3_candle")))
			{
		...
		else if (lever_name.equals("w3_open_door_11"))
		{
			...
			{
				if (m_hero.steal("w3_door_key_11"))
				{
	...
	protected void on_room_entered(String room_door_name) {
		...
		else if (room_door_name.equals("w3_part_2_door"))
		{
			start_clock();
		}
	}
	...
	protected boolean on_trigger_activated(String trigger_name) 
	{
		...
		else if (trigger_name.equals("w2_disable_teleport"))
		{
			...
			if (!m_w2_disable_teleport)
			{
				...
				if (m_hero.steal("w2_close_trap_door_1"))
				{
		...
		{
			if (!m_hero.owns("w2_steel_chalice"))
			{
		...
		else if (trigger_name.equals("w2_display_part_2_key"))
		{
			...
			if (m_hero.owns("w2_iron_chalice"))
			{
		...
		else if (trigger_name.equals("w2_speed_bonus_2"))
		{
			...
			if (display_speed_bonus("w2_speed_bonus_2_1", 180))
			{
		...
		{
			if (m_hero.owns("w2_gold_chalice") && m_hero.owns("w2_steel_chalice") 
					&& m_hero.owns("w2_iron_chalice"))
			{
		...
		{
			if (m_w3_is_crypt_door_closed && m_hero.owns("w3_herb"))
			{
		...
		else if (trigger_name.equals("w3_open_teleport_trap"))
		{
			...
			if (!is_lever_activated("w3_kill_spikes_6") && (!is_lever_activated("w3_part_3_lever")))
			{
	...
	{
		boolean rval = (m_hero.steal(gem_name));
