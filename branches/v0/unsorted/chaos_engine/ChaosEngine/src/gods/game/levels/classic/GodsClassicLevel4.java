...
public class GodsClassicLevel4 extends GodsLevel 
{
	...
	private void display_ten_lives()
	{
		...
		{
			create_bonus(x, y, "extra_life","end_extra_life_"+(i+1));
			x += delta_x;
		}
	...
	{
		if (bonus.get_name().startsWith("end_extra_life_"))
		{
			m_nb_lives_taken++;
		...
		else if (bonus.get_name().equals("w2_shop"))
		{
			...
			summon_shopkeeper(GodsClassicLevel3.SHOP_CONTENTS_W3_END);
		}
	...
	protected void on_button_pressed(String button_name) 
	{
		if (button_name.equals("w2_button_1"))
		{
			...
			if (m_hero.steal("w2_treasure_key"))
			{
		...
		{
			if (m_hero.steal("w3_trap_door_key"))
			{
			...
			LeverActivationState state) 
	{
		...
		{
			if ((state == LeverActivationState.DEACTIVATED) && m_hero.owns("w1_world_key"))
			{
				if (m_hero.steal("w1_trap_door_key_1"))
				{
		...
		{
			if (m_hero.owns("w2_pestle") && m_hero.owns("w2_gold_bowl"))
			{
		...
		else if (lever_name.equals("w2_open_pestle_out_door"))
		{
			if (is_lever_activated(lever_name))
			{
				if (m_hero.steal("w2_move_block_pestle_trap_door_key"))
				{
	...
	private boolean on_trigger_activated_world_1(String trigger_name)
	{
		...
		{
			display_speed_bonus("w1_speed_bonus", 120);
		}
		...
		{
			rval = m_hero.steal("w1_trap_door_key_4");
	...
	private boolean on_trigger_activated_world_2(String trigger_name)
	{
		...
		else if (trigger_name.equals("w2_display_world_key"))
		{
			...
			if (rval)
			{
				m_hero.steal("w2_pestle");
				m_hero.steal("w2_lamp");
				m_hero.steal("w2_gold_bowl");
	...
	{
		return m_hero.owns("w2_pestle") && m_hero.owns("w2_lamp") && m_hero.owns("w2_gold_bowl");
	}
