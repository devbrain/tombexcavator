...
public class GodsClassicLevel2 extends GodsLevel 
{
	...
	protected boolean on_bonus_taken(GfxObject bonus) 
	{
		...
		if (name.equals("w2_end_shopkeeper_token"))
		{
			...
			summon_shopkeeper(SHOP_CONTENTS_W2_END);
		}
	...
	protected void on_lever_activated(GfxObject lever,
		...
		{
			if (m_hero.steal("w2_bottom_room_key_1"))
			{
		...
		else if (lever_name.equals("w3_move_block_lever_2"))
		{
			...
			case FIRST_ACTIVATED:
				...
				break;
			case DEACTIVATED:
				if (m_hero.steal("w3_platform_1_key"))
				{
		...
		{
			if (m_hero.owns("w3_door_key_1") && (m_w3_block_1_move == 1))
			{
	...
	private boolean on_trigger_activated_1(String trigger_name)
	{
		...
		{
			display_health_and_lives_bonus("w1_lives_bonus_1",3);
		...
		else if (trigger_name.equals("w1_gmt_key_1"))
		{
			...
			if (m_hero.owns(W1_BAIT_KEY) && is_button_activated("w1_thief_riddle_button"))
			{
		...
		{
			rval = m_hero.owns("ice_gem") && is_hostile_alive("w1_pit_spikes");
		...
		{
			boolean owns_ice_gem = m_hero.owns("ice_gem");
			boolean owns_water_gem = m_hero.owns("water_gem");
			boolean owns_fire_gem = m_hero.owns("fire_gem");
			...
			{
				if ( m_hero.steal("ice_gem") || m_hero.steal("water_gem") || m_hero.steal("fire_gem") )
				{
	...
	private boolean on_trigger_activated_2(String trigger_name, boolean rval_in)
	{
		...
		if (rval)
		{
			...
			else if (trigger_name.equals("w2_chalice_room_treasure_trigger"))
			{
				...
				if (m_hero.owns("steel_chalice") && m_hero.owns("iron_chalice") && m_hero.owns("gold_chalice"))
				{
	...
	protected boolean on_trigger_activated(String trigger_name) 
	{
		...
		else if (trigger_name.equals("w3_part_2_start_clock"))
		{
			start_clock(); // for speed bonus 1
		}
		...
		{
			if (m_hero.owns("w3_room_platform_key"))
			{
		...
		else if (trigger_name.equals("w3_kill_spikes_cross"))
		{
			...
			{
				kill("w3_spike_cross_1");
				kill("w3_spike_cross_2");
			}
			else
			{
				rval = false;
		...
		else if (trigger_name.equals("w3_speed_bonus_2"))
		{
			...
			if ((get_clock() < 230) && (m_hero.owns("w3_cross")))
			{
		...
		{
			if (m_hero.steal("w3_candle_1"))
			{
				display("w3_boss_door_key");
				if (m_hero.owns("w3_cross"))
				{
		...
		{
			if (m_hero.owns("candle_1") && m_hero.owns("candle_2"))
			{
