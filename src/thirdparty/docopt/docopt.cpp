...
nlohmann::json to_json(const PatternList& plist);

nlohmann::json to_json(const BranchPattern* bp)
{
    if (bp->children().size() == 1)
    {
        return to_json(bp->children());
    }

    auto j = nlohmann::json::object();
    j["branch"] = to_json(bp->children());
    return j;
}

nlohmann::json to_json(const LeafPattern* lp)
{
    auto j = nlohmann::json::object();
    j["name"] = lp->name();
    j["type"] = lp->getValue().kindAsString();
    return j;
}

nlohmann::json to_json(const Pattern* p)
{
    if (const LeafPattern* lp = dynamic_cast<const LeafPattern*>(p))
    {
        return to_json(lp);
    }
    if (const BranchPattern* bp = dynamic_cast<const BranchPattern*>(p))
    {
        return to_json(bp);
    }
...
nlohmann::json to_json(const PatternList& plist)
{
    if (plist.size()==1)
    {
        return to_json(plist.front().get());
    }

    auto j = nlohmann::json::array();
    for (const auto& v : plist)
    {
        const Pattern* p = v.get();
        j.push_back(to_json(p));
    }
    return j;
}

nlohmann::json to_json(const Required& r)
{
    auto j = nlohmann::json::object();
    j["options"] = to_json(r.children());
    return j;
}

DOCOPT_INLINE
std::string  docopt::docopt_to_json(const std::string& doc)
{
    Required pattern;
    ...
    nlohmann::json j = to_json(pattern);
