...
{
    bool ea_events::on_start_group(std::istream& is, chunk_type type, [[maybe_unused]] uint64_t offset, [[maybe_unused]] std::size_t size)
    {
        if (type == fourcc("FORM"))
        {
            read_chunk_type(is);
    ...
    // -------------------------------------------------------------------------------------------------
    void ea_events::on_end_group([[maybe_unused]] chunk_type type)
    {

    }
    // -------------------------------------------------------------------------------------------------
    chunk_type ea_events::read_chunk_type(std::istream& is) const
    {
        uint32_t dword;
