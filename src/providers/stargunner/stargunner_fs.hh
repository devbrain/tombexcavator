#ifndef STARGUNNER_FS_HH
#define STARGUNNER_FS_HH
...
namespace stargunner
{
    ...
    {
    public:
        explicit file_inode(std::shared_ptr<dlt_archive> dlt, const fat_entry& fat);
    private:
    ...
    private:
        std::shared_ptr<dlt_archive> m_dlt;
        fat_entry m_fat;
    };
    // -------------------------------------------------------------------------------------------
    ...
    {
    public:
        explicit directory_inode(std::shared_ptr<dlt_archive> dlt, const std::string dir_name);
    private:
        archive::read_only_inode* find_by_name(const char* name) override ;
        uint64_t size() override ;
        archive::read_only_file* open_read_only() override ;
