...
const char *tmpfname = "$tmpfil$.exe";
const char *backup_ext = ".olz";
...
int reloc90 (FILE *ifile,FILE *ofile,long fpos) 
{
	...
	c = my_getw (ifile) & 0xFFFF;
	for(; c>0; c--) 
	  {
	    rel_off = my_getw(ifile);
	    my_putw (rel_off, ofile);
	    my_putw (rel_seg, ofile);
	    rel_count++;
	  }
	rel_seg += 0x1000;
    } 
	...
	if ( !getbit(&bits) ) 
	  {
	    ...
	    len |= getbit (&bits);
	    len += 2;
	    span = (my_getc (ifile) & 0xFFFF) | 0xff00;
	  } 
	else 
	  {
	    span = (BYTE) (my_getc (ifile) & 0xFFFF) ;
	    len  = (my_getc (ifile) & 0xFF);
	    span |= ((len & ~0x07)<<5) | 0xe000;
	    len = (len & 0x07)+2;
	    if (len==2) 
	      {
		len = (my_getc (ifile) & 0xFFFF);
