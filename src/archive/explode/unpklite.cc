...
#include "archive/explode/struct_reader.hh"
...
namespace
{
  ...
  // ===================================================================
  void adjust_length_code_2000 (uint16_t& length_code, archive::explode::bit_reader& f, bool uncompressed_region)
  {
    // 4627
    while (true)
      {
  ...
  // -------------------------------------------------------------------
  void adjust_length_code_n2000 (uint16_t& length_code, archive::explode::bit_reader& f, bool uncompressed_region)
  {
    // 474e
    while (true)
      {
  ...
  typedef void (*adjust_length_code_fn) (uint16_t& length_code, 
					 archive::explode::bit_reader& f,
					 bool uncompressed_region);

  uint16_t get_base_offset (archive::explode::bit_reader& f)
  {
    // 4b05
    while (true)
      {
    ...
    throw archive::decoder_error ("should not be here");
    return 0;
#endif
  }
  // ===================================================================
  void build_rellocs (uint16_t h_pklite_info,
		      archive::explode::struct_reader <uint32_t>& fr,
		      std::vector <archive::explode::rellocation>& rellocs)
  {

	archive::explode::struct_reader <uint16_t> f(fr.input ());
	...
	while (true)
	  {
		...
		rellocs.push_back (archive::explode::rellocation (seg, rel));
		relocs_count++;
		has_bytes++;
	      }
	  }
