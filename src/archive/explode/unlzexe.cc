...
static void build_rellocs_90 (archive::input& file, std::vector <archive::explode::rellocation>& rellocs)
{
	archive::explode::byte_reader f(file);
  int16_t seg = 0;
  do
    {
	  uint16_t t = f.word();
      ...
      for (; c>0; c--)
	{
	  ...
	  rellocs.push_back (archive::explode::rellocation (static_cast <uint16_t> (seg), offs));
	}
...
// ----------------------------------------------------------------
static void build_rellocs_91 (archive::input& file, std::vector <archive::explode::rellocation>& rellocs)
{
  int16_t seg  = 0;
  int16_t offs = 0;
  int16_t span = 0;
  ...
  while (true)
      ...
      if (span == 0)
	{
      ...
      offs &= 0x0f;
      rellocs.push_back (archive::explode::rellocation (static_cast <uint16_t> (seg), static_cast <uint16_t>(offs)));
    };
}

static uint32_t unpak_code(archive::explode::output_exe_file& oexe, archive::input& input, uint32_t offset)
{
  input.seek(offset);
    archive::explode::bit_reader bitstream(input);
