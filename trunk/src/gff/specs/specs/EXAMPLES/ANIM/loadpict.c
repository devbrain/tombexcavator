...
/** LoadPict.c **************************************************************
 *
 * Load an ILBM raster image file into an existing bitmap.
 * This is a slightly modified version of ReadPict.c dated 23-Jan-86.
 * 
 * Modified by Gary Bonham, Sparta, Inc.  25 Apr 1986
 * Further mods on 26-Jul-86 to permit it to read only the initial
 * bitmap for ANIM files.  Use LoadAnim.c to read entire anim files.
 *
...
/*          if (iffp == IFF_DONE) iffp = IFF_OKAY;*/
   ...
   do {
      switch (iffp = GetFChunkHdr(&formContext)) {
         case ID_ANHD: {
	    iffp = GetANHD(&formContext, &animHdr);
            Pop = animHdr.operation;
	    break;
         }
         case ID_BMHD: {
         ...
         case ID_CMAP: {
            ...
            iffp = GetCMAP(&formContext, (WORD *)&ilbmFrame.colorMap[0]
         ...
         case ID_BODY: {
            ...
            plane0 = gBM->Planes[0]; /* save ptr to existing bitmap  - GB 25 apr 86*/
            InitBitMap(gBM,(long)nPlanes,(long)ilbmFrame.bmHdr.pageWidth
                      ,(long)ilbmFrame.bmHdr.pageHeight);
            plsize = RowBytes((long)ilbmFrame.bmHdr.pageWidth)
                            * (long)ilbmFrame.bmHdr.pageHeight;
              /* Assume all planes were allocated contiguously.
               * Here we partition up the memory for the required bitmaps
               * on the assumption that there was enough allocated.
               */
            if (plane0) {  /* GB - 25 apr 86 */
               for (i = 0; i < nPlanes; i++) gBM->Planes[i] = plane0 + plsize*i;
               iffp = GetBODY(&formContext,gBM,0L,&ilbmFrame.bmHdr
                      ,bodyBuffer,bufSz);
               *giFrame = ilbmFrame;  /* Copy fields to client's frame.*/
               if (iffp == IFF_OKAY) iffp = IFF_DONE;   /* Eureka */
            }
            else iffp = CLIENT_ERROR;   /* RAM not allocated for the bitmap */
            break;
         }
         case END_MARK: {
            Pop = 0;
...
GroupContext *parent;
{
   ...
   if (parent->subtype != ID_ILBM) return(IFF_OKAY);   /* just continue scaning the file */
