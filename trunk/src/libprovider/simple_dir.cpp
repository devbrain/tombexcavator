...
namespace provider 
{
  ...
  // -----------------------------------------------------------------------
  std::size_t simple_dir_c::read (std::size_t entry_num, std::string& name,
				  vfs::inode_num_t& inode_num)
  {
    ...
    fs_iterator_t j = m_pimpl->m_fs_map.find (inode_num);
    name = j->second->name ();
    return 1;
  }
  // -----------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------
  std::string simple_dir_c::name () const
  {
  ...
  bool simple_dir_c::_add_fs_object (vfs::fs_object_c* fs_obj)
  {
    ...
    const std::string name = fs_obj->name ();
