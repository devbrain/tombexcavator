...
public abstract class GridLayer implements Describable
{
	...
	protected GfxFrame m_tile_behind = null;
	...
	protected void load(ParameterParser fr, GfxPalette palette) throws IOException
	{
		...
		String tb = fr.readString("tile_behind",true);		
		
		if (tb != ParameterParser.UNDEFINED_STRING)
		{
			GfxFrameSet gf = palette.lookup_frame_set(tb);
			
			if (gf != null)
			{
				m_tile_behind = gf.get_first_frame();
			}
		}
		init_dim_params(palette);
		init_grid();
		...
		for (int i = 0; i < m_nb_rows; i++)
		{
			fr.startBlockVerify("row");
			
			for (int j = 0; j < m_nb_cols; j++)
			{
				
				int tile_counter = fr.readInteger("counter");
				String tile_name = fr.readString("name");
				...
				if (tile_counter > 0)
				{
					...
					if (gfs != null)
					{					
						m_data[j][i] = gfs.get_frame(tile_counter);
					}
				}
			}
	...
	/* (non-Javadoc)
	 * @see base.grid.FrameLayer#serialize(sys.ParameterParser)
	 */
	public void serialize(ParameterParser fw) throws IOException 
	{
		...
		fw.write("tile_behind",
				(m_tile_behind != null) ? m_tile_behind.get_source_set().get_name() : ParameterParser.UNDEFINED_STRING);
			
		for (int j = 0; j < m_nb_rows; j++)
		{
			fw.startBlockWrite("row");
			...
			for (int i = 0; i < m_nb_cols; i++)
			{
				...
				int tile_counter = gf == null ? 0 : gf.get_counter();
				
				fw.write("counter", tile_counter);
				fw.write("name", tile_name);
			}
