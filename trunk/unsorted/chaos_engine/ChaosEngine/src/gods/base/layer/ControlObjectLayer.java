...
public class ControlObjectLayer extends SparseLayer<ControlObject> {
	...
	protected ControlObject add_item(ParameterParser fr, int x_res, int y_res, GfxPalette palette, boolean scale2x) throws IOException 
	{
		ControlObject rval = new ControlObject(fr,x_res,y_res,scale2x);
	...
	protected void load(ParameterParser fr, boolean scale2x) throws IOException {
		super.load(fr, null, scale2x);
	}
	...
	public ControlObjectLayer(ParameterParser fr,int x_res, int y_res, boolean scale2x) throws IOException
	{
		super(x_res,y_res);
		load(fr,scale2x);
	...
	public void set_hostile_parameters(HostileWaveParametersSet hwps, GfxPalette lp)
	{
	...
	public void editor_render(Graphics g) 
	{
		...
		for (ControlObject co : m_object_list)
		{
			if (m_view_filter.get(co.get_type()) != null)
			{
				...
				if (render_outline)			co.editor_render(g);
			}
		}
	}
