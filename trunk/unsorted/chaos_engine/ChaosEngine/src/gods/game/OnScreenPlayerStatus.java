...
public class OnScreenPlayerStatus implements Renderable
{
	...
	private Hero m_hero;
	private static final int TIME_BEFORE_BONUS_DISPLAY = 5000;
	...
	public void set_bonus_value(int bonus_value)
	{
		m_world_end_osd.set_bonus_value(bonus_value);
	}
			...
			GfxFrame gold_bag_icon, Hero hero, Hostile boss, SfxSet sfx_set, int width)
	{
		...
		try 
		{
			m_lives_and_score_font = new GameFont(DirectoryBase.get_font_path() + "small_letters", true);
			m_big_font = new GameFont(DirectoryBase.get_font_path() + "big_letters");
		} catch (IOException e) 
		{
			e.printStackTrace();
		}
	...
	public void render(Graphics2D g)
	{
		...
		if (Debug.active)
		{
			...
			String time = (m_global_timer / 60000) + ":" + ((seconds < 10) ? "0"+seconds : seconds);

			m_lives_and_score_font.write_line(g, time, m_width-80, 0, 0, false, false);
		}
	...
	public enum EndType { LEVEL_END, WORLD_END, GAME_END }
			...
			if (m_state_counter > TIME_BEFORE_BONUS_DISPLAY + 4000)
			{
				if (m_bonus_value > 0)
				{
					...
					if (m_bonus_value > 0)
					{
						...
						m_hero.add_score(transfer_unit,false);
					}
					
					if (m_bonus_value <= 0)
					{
						m_hero.add_score(m_bonus_value,false);
						m_bonus_value = 0;
