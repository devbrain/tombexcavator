...
namespace ChaosEngineViewer {
  public class SpriteBin : IDisposable {
    ...
    public SpriteBin (VersionEnum version, string fileName, int type, int nb, Palette palette, int paletteOffset, MasksEnum mask, int cols) {
      _fileName = fileName;
    ...
    public SpriteBin (VersionEnum version, string fileName, string title, int? archive, int offset, int nb, int width, int height, int bp, ModesEnum mode, int interleave, Palette palette, int paletteOffset, MasksEnum mask, int cols) {
      _fileName = fileName;
      _widthSingle = width;
      _heightSingle = height;
      _title = title;
      _bitmaps = CreateBitmaps (version, fileName, archive, offset, nb, width, height, bp, mode, interleave, palette, paletteOffset, mask);
      CreateSummaryBitmap (cols);
    }
    ...
    public static Bitmap [] CreateBitmaps (VersionEnum version, string fileName, int? archive, int offset, int nb, int width, int height, int bp, ModesEnum mode, int interleave, Palette palette, int paletteOffset, MasksEnum mask) {
    ...
    private static Bitmap ReadChunky_4bp (Dump dump, int src, int width, int height, Palette palette, int paletteOffset, MasksEnum mask) {
      uint [] buffer;
      ...
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x += 2) {
          byte b = (byte) dump.ReadByte (src);
          src++;
          byte color1 = (byte) ((b >> 4) & 0x0F);
          byte color2 = (byte) ((b >> 0) & 0x0F);
          WriteColor (buffer, dst, palette.GetRawARGB (color1 + paletteOffset), color1 == 0, mask);
          dst++;
          WriteColor (buffer, dst, palette.GetRawARGB (color2 + paletteOffset), color2 == 0, mask);
          dst++;
        }
      }
      return (bitmap);
    ...
    private static byte GetPixelColor (Dump dump, int address, int x, int y, int interleaveSize, int scanlineStride, int bitplaneStride, int nbBitplanes) {
      byte color = 0;
      ...
      for (int bitplane = 0; bitplane < nbBitplanes; bitplane++) {
        ...
        if ((dump.ReadByte (address + offset) & (1 << (7 - (x % 8)))) != 0) {
    ...
    private static Bitmap ReadBitmap (Dump dump, int address, int width, int height, int interleaveSize, int scanlineStride, int bitplaneStride, int nbBitplanes, Palette palette, int paletteOffset, MasksEnum mask) {
      uint [] buffer;
      Bitmap bitmap = AllocBitmap (width, height, out buffer);
      ...
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
          byte color = GetPixelColor (dump, address, x, y, interleaveSize, scanlineStride, bitplaneStride, nbBitplanes);
          WriteColor (buffer, dst, palette.GetRawARGB (color + paletteOffset), color == 0, mask);
          dst++;
        }
      }
    ...
    public int WidthSingle {
      get {
        return (_widthSingle);
      }
    }

    public int HeightSingle {
      get {
        return (_heightSingle);
      }
    }
