...
import editor.DataModifier;

import javax.swing.JDialog;

import base.*;
import base.HostileWaveParameters.AppearingDelay;
import editor.ComboSelectorDialog;
import sys.JMultiLineToolTip;
...
public class HostileWaveDialog extends JDialog implements DataModifier<HostileWaveParameters> {
	...
	private JCheckBox jShootShieldCheckBox = null;
	...
	public void data_to_gui() 
	{
		if (m_data != null)
		{
			...
			String package_name = "game.characters.hostiles";
			...
			getJNbEnemiesTextField().setText(""+m_data.object_to_drop.size());
	...
	{
		int nb_items = m_data.object_to_drop.size();
		...
		for (ObjectToDrop o : m_data.object_to_drop)
		{
	...
	private void update_dropped_object_list(int nb_items)
	{
		int current_nb_items = m_data.object_to_drop.size();
		if (nb_items < current_nb_items)
		{
			for (int i = 0; i < current_nb_items - nb_items + 1; i++)
			{
				// objects removed
				
				m_data.remove(nb_items+i-1);
			}
		} else if (current_nb_items < nb_items)
		{
			for (int i = 0; i < nb_items - current_nb_items; i++)
			{
				// objects added
				m_data.add(null,null,null);
			}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLabel1611.setBounds(new Rectangle(249, 325, 89, 16));
			...
			jLabel161.setBounds(new Rectangle(246, 285, 100, 26));
			jLabel161.setToolTipText("Jumps when more or less close to the wall");
			jLabel161.setText("Jump threshold");
			jLabel16 = new JLabel();
			jLabel16.setBounds(new Rectangle(246, 248, 97, 27));
			jLabel16.setText("Jump height");
			jLabel15 = new JLabel();
			jLabel15.setBounds(new Rectangle(259, 211, 110, 22));
			jLabel15.setText("attack distance");
			jLabel14 = new JLabel();
			jLabel14.setBounds(new Rectangle(251, 172, 121, 28));
			...
			jContentPane.add(getJShootShieldCheckBox(), null);
	...
	private JTextField getJNbEnemiesTextField() {
		if (jNbEnemiesTextField == null) {
			...
			jNbEnemiesTextField.setBounds(new Rectangle(107, 254, 65, 24));
			jNbEnemiesTextField.setToolTipText("Number of hostiles in the wave");

			jNbEnemiesTextField.addCaretListener(new javax.swing.event.CaretListener() {
				public void caretUpdate(javax.swing.event.CaretEvent e) 
				{
					try {
						String s = getJNbEnemiesTextField().getText();

						int nb_enemies = Integer.parseInt(s);

						update_dropped_object_list(nb_enemies);
					} 
	...
	private void set_association_context(int index)
	{
		getJClassNameList().setSelectedIndex(index);
		getJInstanceNameList().setSelectedIndex(index);
		getJHostileNameList().setSelectedIndex(index);
			     ...
			     public void mouseClicked(MouseEvent e) {
			         ...
			         {			        	 
			        	 GfxFrameSet go = (GfxFrameSet)ComboSelectorDialog.show((JFrame)(HostileWaveDialog.this.getParent()), 
			        		 ...
			        		 ObjectToDrop o = m_data.object_to_drop.elementAt(index);
			        		 if (o == null)
			        		 {
			        			 o = new ObjectToDrop();
			        		 ...
			        		 m_data.object_to_drop.set(index, o);
			        		 update_dropped_object_list();
			...
			MouseListener mouseListener = new MouseAdapter() {
			     public void mouseClicked(MouseEvent e) {
			         ...
			         {			        	 
			        	 ...
			        	 Collection<GfxObject> cog = m_data.level.get_object_layer().get_items(GfxFrameSet.BONUSES,false);
	...
	private JComboBox getJAttackDistanceComboBox() {
		if (jAttackDistanceComboBox == null) {
			...
			jAttackDistanceComboBox.setBounds(new Rectangle(383, 211, 164, 23));
	...
	/**
	 * This method initializes jShootShieldCheckBox	
	 * 	
	 * @return javax.swing.JCheckBox	
	 */
	private JCheckBox getJShootShieldCheckBox() {
		if (jShootShieldCheckBox == null) {
			jShootShieldCheckBox = new JCheckBox()
			{
				public static final long serialVersionUID = 1L;
			...
			jShootShieldCheckBox.setBounds(new Rectangle(10, 400, 136, 24));
			jShootShieldCheckBox.setText("Shoot shield");
			jShootShieldCheckBox.setToolTipText("If set, shoot damage to hostile is divided by 5");
		}
		return jShootShieldCheckBox;
	}
	...
	private JComboBox getJJumpHeightComboBox() {
		if (jJumpHeightComboBox == null) {
			...
			jJumpHeightComboBox.setBounds(new Rectangle(365, 247, 189, 27));
		}
	...
	private JComboBox getJJumpThresholdComboBox() {
		if (jJumpThresholdComboBox == null) {
			...
			jJumpThresholdComboBox.setBounds(new Rectangle(367, 285, 187, 25));
		}
	...
	private JComboBox getJJumpWidthComboBox() {
		if (jJumpWidthComboBox == null) {
			...
			jJumpWidthComboBox.setBounds(new Rectangle(367, 324, 188, 25));
		}
