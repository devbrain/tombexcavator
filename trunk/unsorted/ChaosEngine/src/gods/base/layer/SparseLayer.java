...
{	
	...
	protected abstract T add_item(ParameterParser fr, int x_res, int y_res, GfxPalette palette, boolean scale2x) throws IOException;

	protected LinkedList<T> m_object_list = new LinkedList<T>();
	
	protected void load(ParameterParser fr, GfxPalette palette, boolean scale2x) throws IOException 
	{	
		...
		{
			T o = add_item(fr,m_x_resolution,m_y_resolution,palette,scale2x);
			// game mode: generate a unique name for unnamed objects
			// or hash sets & sectors won't work
			if ((scale2x) && (o.get_name().equals("")))
			{
