...
public class GfxObject extends Locatable implements LayerObject, Comparable<Object>
{
	@Override
	...
	private LinearMove m_move_x = new LinearMove();
	...
	public void spawn_linked_objects(LevelData ld)
	{
		if (m_linked_objects != null)
		{
			...
			int nb_items = m_linked_objects.size();
			int i = 0;
			
			for (GfxObject go : m_linked_objects)
			{
				int old_x = go.get_x();
		...
		while (m_timer > FALL_UPDATE_RATE)
		{
			...
			if (in_screen)
			{
				...
				if (m_move_x.active)
				{
					...
					if (m_move_x.counter >= m_move_x.rate)
					{
						m_move_x.counter = 0;

						m_x += m_move_x.speed;

						if (((m_move_x.speed <= 0) && (m_x <= m_move_x.pos_end)) || ((m_move_x.speed >= 0) && (m_x >= m_move_x.pos_end)))
						{
							m_x = m_move_x.pos_end;
							m_move_x.active = false;
						}
					}
				}
	...
	public GfxObject(ParameterParser fr, int x_resolution, int y_resolution, 
			GfxPalette palette, boolean game_scale2x) throws java.io.IOException
	{		
		if (game_scale2x)
		{
	...
	public void set_out_of_chest_move(Locatable chest_position, int x_move_rate, int x_speed, int y_move_rate, int y_speed)
	{
		m_move_x.init(get_x(),x_speed,x_move_rate);
		m_move_y.init(get_y(),y_speed,y_move_rate);
		
		
		m_x = chest_position.get_x();
	...
	{
		return (!m_move_x.active && !m_move_y.active);
	}
}
