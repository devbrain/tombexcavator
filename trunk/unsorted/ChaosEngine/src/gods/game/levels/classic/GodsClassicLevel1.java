...
{	
	...
	protected boolean on_trigger_activated(String trigger_name) 
	{
		...
		{
			display_health_and_lives_bonus("health_and_lives_bonus_w1",2);
			
			display_help_health_bonus("help_health_w1");
			
			display_help_lives_bonus("help_extra_life_w1");
		...
		{
			boolean o1 = m_hero.owns("pot_for_stores_1");
			boolean o2 = m_hero.owns("pot_for_stores_2");
			...
			if (o1)
			{
				...
				m_hero.steal("pot_for_stores_1");
			}
			if (o2)
			{
				rval = true;
				display("world_key_w2");
				m_hero.steal("pot_for_stores_2");
			}
		}
		else if (trigger_name.equals("w2_start_clock"))
		{
			// for speed bonus
			start_clock();
		}
		...
		{
			if (m_hero.owns("pot_for_stores_1") && m_hero.owns("pot_for_stores_2"))
			{
		...
		{
			// create last hostile only if score is high enough (player monitor bullshit)
			if (m_hero.get_score() >= 20000)
			{
				create_hostile("gm_w2_end");
			}
			
			// does not work if flasks are taken instead
			
			boolean o1 = m_hero.owns("pot_room_pot_1");
			boolean o2 = m_hero.owns("pot_room_pot_2");
			if (o1 && o2)
			{
				display("special_health_bonus_w2");
				m_hero.steal("pot_room_pot_1");
				m_hero.steal("pot_room_pot_2");
			}
		...
		else if (trigger_name.equals("w2_lives_bonus"))
		{
			...
			{
				display_lives_bonus("w2_lives_bonus",2);
			}
		}

		else if (trigger_name.equals("gmt_after_teleport_w2_21"))
		{
			rval =  m_hero.owns("pot_for_stores_2");
		...
		{
			if ((m_hero.get_score() > 80000) && (get_clock() < 140) && (m_hero.get_nb_lives() > 2))
			{
		...
		else if (trigger_name.equals("w3_start_clock_2"))
		{
			start_clock();
		}
		else if (trigger_name.equals("display_boss_trap_door_key"))
		{
			rval =  m_hero.owns("world_key_w3");
		...
		else if (trigger_name.equals("special_bonus_boss"))
		{
			...
			{
				if ((m_boss_dead) && (display("w3_end_chest")))
				{
					rval = true;
	...
	protected void on_button_pressed(String button_name) 
	{
		...
		else if (button_name.equals("open_trap_4_secret"))
		{
			...
			if (m_hero.owns("trap_door_key_1_w2") && (m_secret_button_w1_pressed))
			{
	...
	protected void on_lever_activated(GfxObject lever, LeverActivationState state) 
	{
		...
		} else if (lever_name.equals("spike_kill_lever_2"))
		{
			...
			{
				if (!m_short_cut_enabled && m_hero.owns("treasure_key_w1"))
				{
		...
		{			
			if (is_lever_activated("world_door_1_lever"))
			{
		...
		else if (lever_name.equals("w3_treasure_room_open_lever"))
		{
			...
			{
				if (m_hero.steal("w3_treasure_key_1"))
				{
	...
	protected boolean on_bonus_taken(GfxObject bonus)
	{
		...
		else if (name.equals("w3_shop_token"))
		{
			...
			summon_shopkeeper(SHOP_CONTENTS_W3_START);
		...
		else if (name.equals("w3_end_shop_token"))
		{
			...
			summon_shopkeeper(SHOP_CONTENTS_W3_END);
	...
	protected void on_door_change_state(String door_name, boolean is_open) 
	{
		...
		{
			display_speed_bonus("w2_speed_bonus_pot",60);
		}
