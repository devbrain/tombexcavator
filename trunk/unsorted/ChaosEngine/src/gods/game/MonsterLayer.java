...
public class MonsterLayer implements Renderable
{
	...
	public Hostile closest_hostile(Locatable l, int in_screen_margin)
	{
	...
	private Hostile closest_hostile(Locatable l, Collection<Hostile> hl, int in_screen_margin)
	{
		Hostile rval = null;
	...
	private void process_active_items(LinkedList<Hostile> l, long elapsed_time)
	{
		...
		while (it2.hasNext())
		{
			...
			case EXPLODING:
			{
				GfxObject go = m.get_dropped_item();
				if (go != null)
				{
					go.set_visible(true);
