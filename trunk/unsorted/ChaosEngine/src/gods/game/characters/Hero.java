...
public class Hero extends LivingCharacter
{
	...
	private static final int JUMP_RESOLUTION = 6;
	...
	private enum PositionState { FRONT, BACK, WALK, JUMP, CLIMB, CROUCH, FALL }
	...
	public boolean steal(String name)
	{
	...
	private class ItemHolder
	{
		...
		void init()
		{
			...
			current_pos = 0;
			m_alt_jumps = GameOptions.instance().get_control_type() == GameOptions.ControlType.ALT_JUMPS;
	...
	public void hurt(int damage_points)
	{
		if (!is_invincible())
		{
			...
			if (m_shield_level > 0)
			{
				int divisor = m_shield_level;
				
				if (m_hurt_from_fall)
				{
					// first shield only affects falls
					divisor++;
				}
				
				actual_damage_points /= divisor;
	...
	private class Jump extends ParabolicMobileBehaviour
	{		
		private boolean y_way_blocked;
		private boolean allow_shoot;
		private int blocked_x_index;
		private int x_jump;
		...
		{
			int ha = (m_x_amplitude / 4);
			boolean rval = (m_index > ha);
			// && (blocked_x_index > ha);
			
			if (rval && from_ladder)
			{
				if (blocked_x_index < m_x_amplitude)
				{
					rval = false;
				}
				/*else
		...
		void init()
		{
			...
			int height = (m_giant_jump_timer > 0 ? m_vert_resolution * 4 : m_vert_resolution * 2);
			
			init((int)m_x,(int)m_y, get_walk_direction(), JUMP_WIDTH, height, JUMP_WIDTH, 0);

			x_jump = m_level_data.get_grid().get_rounded_x((int)m_x,false);

			y_way_blocked = false;
			allow_shoot = false;
			blocked_x_index = m_x_amplitude;
		...
		void update(long elapsed_time)
		{
			...
			if (!y_way_blocked)
			{
				if ((m_index < 12) || (m_index > m_x_amplitude - 12))
				{
					if (from_ladder)
					{
						if (m_index < 12)
			...
			double index_variation = elapsed_time / JUMP_RESOLUTION;
			
			update(index_variation);
			...
			{
				// land on the upper platform in some cases (l2 classic bonus section)
				
				if ((m_index > m_x_amplitude - 12) && (!is_lateral_way_blocked_on_boundary(x, m_y - m_vert_resolution)))
				{
				...
				{
					x = m_x;
					if (blocked_x_index > (int)m_index)
					{
						blocked_x_index = (int)m_index;
					}
				}
			}
			...
			boolean key_action, boolean key_fire)
	{
		...
		if (m_crouch_timer >= REFERENCE_FRAME_RATE)
		{
			m_crouch_timer -= REFERENCE_FRAME_RATE;
	...
	public Hero(GameEngine ge, Joystick j)
	{
		m_engine = ge;
		m_money = 0;
		m_score = 0;
	...
	private int JUMP_WIDTH;
	
	public void level_init(LevelData level_data, MonsterLayer ml, 
			StatusBar sb, SfxSet sfx, Rectangle view_bounds)
	{
		...
		JUMP_WIDTH = (21 * m_level_data.get_grid().get_tile_width()) / 8; // 168
	}
	...
	private void update_alive(long elapsed_time)
	{
		...
		boolean key_jump = m_alt_jumps ? m_engine.is_key_down(KeyEvent.VK_SHIFT) : key_up;
		...
		if (m_joystick != null)
		{
			...
			if (!key_fire)
			{
				key_fire = m_joystick.getButtons() != 0;
			}
		...
		if ((!shooting) && (!m_jump.is_active()) && (key_jump) && (current_left_right != -1) && 
				...
				else
				{
					...
					else if (lateral_walk)
					{
						...
						if (m_lateral_walk_timer > lateral_cycle)
						{
							...
							if (!is_lateral_way_blocked_on_boundary(new_x,m_y))
							{
								m_walk_counter++;
								m_current = m_walk[m_right_left].get_frame(m_walk_counter);
	...
	protected void set_invincibility(int seconds)
	{
	...
	public void on_take_object(GfxObject ob, boolean from_shop)
	{
		...
		if (!from_shop && ob.get_source_set().get_type() == GfxFrameSet.Type.money)
		{
			...
			add_score(ob.get_source_set().get_properties().points,true);
		}
	...
	public void set_shield_level(int shield_level) {
		this.m_shield_level = shield_level;
	}
