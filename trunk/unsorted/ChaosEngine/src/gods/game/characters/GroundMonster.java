...
public abstract class GroundMonster extends LeftRightHostile
{
	...
	protected void set_aim(Locatable aim)
	{
		if (m_aim != null)
		{
	...
	private boolean blocked_path_up(int dx, int dy)
	{
		boolean rval = false;
	...
	private boolean blocked_path_down(int dx, int dy)
	{
		boolean rval = false;
	...
	{		
		...
		if (!is_falling() && (faces_aim || is_vertical_way_blocked((int)m_x + m_width/2)))
		{
			...
			if (!is_vertical_way_blocked() && !lateral_way_blocked)  { // on ledge 
				...
				if (dy >= m_tile_height/2)
				{
					// aim is below: do nothing: fall down
					...
					if (on_ledge)
					{
						if (faces_aim || dy < 0)
						{
							if (wd == 1)
							{
								...
								dy = m_jump_right.dy;
								dx = m_jump_right.dx;
								debug("elect ledge jump right: "+jump_on_aim);
							}
							else
							{
								...
								dy = m_jump_left.dy;
								dx = m_jump_left.dx;
								debug("elect ledge jump left: "+jump_on_aim);
							}
							debug("elect ledge jump: "+jump_on_aim);
						}
						else
						{
							debug("no ledge jump because aim lower and does not face aim");
						}
					}
			...
			if (!on_ledge)
			{
				...
				if (dy < -m_tile_height/2)
				{
					...
					{
						// opposed direction to the target: maybe the way is wrong
						//
						// check if target not reachable easily with n "ups" and m "rights" or "lefts"

						jump_on_aim = blocked_path_up(dx,dy);
						if (jump_on_aim)
						{
					...
					if (lateral_way_blocked)
					{
						...
						if (jump_on_aim)
						{
							...
							if (saved_dx * dx < 0)
							{
								...
								jump_on_aim = blocked_path_down(saved_dx, saved_dy);
		...
		if (on_ledge && m_aim_met && m_params.object_held == null)
		{
		...
		else
		{
			...
			if ((dy < -m_level.get_view_bounds().height) && on_ledge)
			{
				do_flip = true;
			}
		}
	...
	public void init(HostileParameters p) {
		...
		m_max_jump_x = m_tile_width * HostileWaveParameters.JUMP_WIDTH_TABLE[m_params.jump_width.ordinal()];
	...
	protected class BaseJump extends Jump
	{
		...
		boolean update(long elapsed_time)
		{
			...
			double step = (m_vertical_speed * elapsed_time) / 60.0;
